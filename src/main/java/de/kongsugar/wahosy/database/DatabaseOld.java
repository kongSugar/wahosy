...
public class DatabaseOld {
		...
		if (conn != null) {
			...
			try {
				...
				while (result.next()) {
					...
					int id = result.getInt(1); // Alternativ:
												// result.getString(1);
					String name = result.getString(2); // Alternativ:
														// result.getString(2);
	 ...
	 */
	public static void insertName(String name_, String manufacturer_, int quantity_, int weight_,
		...
		if (conn != null) {
			try {
				...
				if (result.next()) {
					...
					System.out.format("%5d%20s%20s%10d%10d%10s", id, name, manufacturer, quantity, weight,
							serialnumber);
				}
			} catch (SQLException e) {
				e.printStackTrace();
	 ...
	 */
	public static void updateItem(int id_, String name_, String manufacturer_, int quantity_, int weight_,
			String serialnumber_) {
		conn = getInstance();
